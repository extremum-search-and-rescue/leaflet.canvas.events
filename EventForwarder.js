	...
	const EventForwarder = L.Class.extend({
		...
		_handleMouseMove: function (event) {
			...
			if (_prevTarget && _prevTarget != nextTarget) {
				...
				_prevTarget.dispatchEvent(new MouseEvent('mouseleave', event.originalEvent));
				if (map) {
					L.DomUtil.removeClass(map.getPane('overlayPane'), 'leaflet-interactive');
				}
			}
			...
			if (
				...
				if (nextTarget.classList.value.indexOf('gis-clickable') >= 0 || nextTarget.classList.value.indexOf('gis-tooltip') >= 0) {
					if (map) {
						L.DomUtil.addClass(map.getPane('overlayPane'), 'leaflet-interactive');
		...
		_handleClick: function (event) {
			...
			if (
				...
				let ev;
				if (typeof event.originalEvent === 'MouseEvent' && event.originalEvent._simulated) {
